...
def pynfam_input_check(pynfam_inputs):
    ...
    if rerun not in [0,1]:
        err.append(u"Invalid input for rerun_mode. Options are [0,1].")
    ...
    if beta_type not in [u'+', u'-']:
        err.append(u"Invalid input for beta_type. Options are ['+','-'].")
...
def pynfam_override_check(pynfam_inputs, override_settings):
    ...
    if ctr_type is not None:
        ...
        # Check bounds and half width
        if not (np.isreal(ctr_set["energy_min"]) and np.isreal(ctr_set["energy_max"])):
            err.append(u"Invalid contour bounds. Must be real.")
        if "half_width" in ctr_set.keys():
            hw = ctr_set["half_width"]
            if not np.isreal(hw) and abs(hw.real)>1e-10:
                err.append(u"Invalid input for contour setting 'half_width'. Must be real or imaginary.")

    # Phase Space settings (type is not required, default is ratint)
    psi_options  = [k for k in list(DEFAULTS[u'psi'].keys()) if k != u'PSI']
    if u'psi_approx' in list(psi_set.keys()):
        psi_approx = psi_set[u'psi_approx']
    else:
        psi_approx = DEFAULTS[u'psi'][u'PSI'][u'psi_approx']
    if psi_approx not in psi_options:
        err_str = u"Invalid override setting for 'psi_approx'. Check {:} for options."
        err.append(err_str.format(f_conf))
    psi_defaults  = list(DEFAULTS[u'psi'][u'PSI'].keys())
    psi_defaults += list(DEFAULTS[u'psi'][psi_approx].keys())
    if any(k not in psi_defaults for k in list(psi_set.keys())):
        err_str = u"Invalid override setting for psi: {:}. Check {:} for options."
        err.append(err_str.format(psi_approx, f_conf))
...
def pynfam_init(pynfam_inputs, override_settings):
    ...
    fam_exe = os.path.join(paths.exe, pnfamRun.file_exenompi)
    ...
    if rerun==1:
        ...
        if exst_data:
            ...
            # If existing dirs, make sure they all have hfb solutions
            hfb_solns = [os.path.join(paths.out, p, paths.subdir_hfb, hfbthoRun.file_fam) for p in exst_data]
            missing = [p for p in hfb_solns if not os.path.exists(p)]
            if missing:
                err.append(u"All existing data must include ground state solution.hfb for rerun_mode=1. Missing:")
                err.append(u"\n".join(missing))

            # Dripline mode doesn't make sense in this case
            if dripline:
                err.append(u"Cannot rerun with rerun_mode=1 and dripline active.")
...
def pynfam_mpi_init(pynfam_inputs, nr_calcs, comm, check):
    ...
    if comm_size > 1:
        ...
        if nr_workers <= 0 and not check:
            err.append(u"To few mpi processes reserved. Must have: "+\
