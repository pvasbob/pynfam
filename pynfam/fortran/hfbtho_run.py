...
class hfbthoRun(fortProcess):
    ...
    def soln_dict(self):
        ...
        sd[u'Z-Blk'] = np.sign(self.blocking[1][0])
        sd[u'N']     = self.nucleus[0]
        sd[u'N-Blk'] = np.sign(self.blocking[0][0])
    ...
    #-----------------------------------------------------------------------
    def getEqrpaMax(self, beta, output=None, Be=0.0):
        r"""
        ...
        Args:
            beta (str): The beta decay type. Options are ['+', '-', 'ec'].
            Be (float): The binding energy of captured electron. Only used if beta='ec'.
            output (list of str, str): Input for hfbthoParser.
        ...
        if beta not in [u'+',u'-',u'ec']:
            raise ValueError(u"Valid args for kwarg beta are '+', '-', or 'ec'.")
        ...
        if beta == u'-':
            ...
            eqrpa_max = hfb_lambda[1] - hfb_lambda[0] - DMASS_NH - MEC2 - Be
    ...
    def getEgs(self, beta, output=None):
        ...
        # Gather 2qp transitions. Neglect the transition involving BOTH blocked QP
        # since this is not allowed in the EFA. This is a dictionary b/c
