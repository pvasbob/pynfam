...
class fortProcess(nmlInputs):
    ...
    def runExe(self, stdout_bool=False, statement=None, debug=0):
        ...
        # Default statement is to run exe from current directory
        if statement is None:
            statement = [os.path.join(self.paths.exe,self.exe)]

        # Get the necessary namelist text files
        self.writeNml(self.rundir)
        ...
        if debug == 0:
            ...
            if stdout_bool:
                ...
                output_data, err_data = proc.communicate()
