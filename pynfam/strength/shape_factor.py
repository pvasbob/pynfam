...
class shapeFactor(object):
    """
    An instance of the class shapeFactor contains a set of famStrength objects
    ...
    def __init__(self, strengths):
        ...
        if not all([isinstance(s, famStrength) for s in strengths]):
            raise ValueError(u"Inputs must be strength objects.")
        ...
        if any([s.beta != beta_type for s in strengths]): raise ValueError
    ...
    #                       Shape Factors                          #
    #==============================================================#
    def _calcShapeFactor(self, psi_obj, hfb_gs, zero_neg_str):
        """
        Combine all strengths and phase space into the shape factor. See
        (Mustonen et al., Phys. Rev. C 90, 024308 (2014)).

        This populate most all of the key instance attributes.

        Args:
            psi_obj (phaseSpace): The phase space object.
            hfb_gs (hbfthoRun): The hfbthoRun object for the ground state.
            zero_neg_str (bool): Used the adjusted strength with zeroed negative components.
        """
        ...
        pre = {u'F_K0'   :  Tk[u'K=0'],
               u'GT_K0'  :  Tk[u'K=0']*LAM,
               u'GT_K1'  :  Tk[u'K>0']*LAM,
        ...
        # Get the hfb solution and some constants
        ...
        if self.beta == u'-':
            ...
            Z_change = -1
        Zd     = self.nucleus[1] + Z_change   # daughter Z
        A      = self.nucleus[2]              # parent mass
        rad    = (R0*A**(1.0/3.0)/HBAR_MEC)     # nuclear radius
        gamma1 = np.sqrt(1.0 - (ALPHA*Zd)**2) # gamma1 factor

        # Use real or complex EQRPA grid based on the contour type
        if not self.contour.closed:
            eqrpa = np.real(self.contour.ctr_z)
        else:
            eqrpa = self.contour.ctr_z
        eqrpamin = min(np.real(eqrpa))
        dim = len(eqrpa)

        # Store half_width if it's well defined for the contour
        half_width = np.imag(self.contour.ctr_z[0])
        if self.contour.closed or any(hw != half_width for hw in np.imag(self.contour.ctr_z)):
            half_width = None

        # Check we have all forbidden operators for a given K to avoid computing weird rates
        ...
        k_err    = 3*[False]
        k_err[0] = any([o not in self.genops for o in k0_fops_raw])
        k_err[1] = any([o not in self.genops for o in k1_fops_raw])
        k_err[2] = any([o not in self.genops for o in k2_fops_raw])

        # Compute the phase space
        ps = pd.DataFrame()
        wmax = 1.0 + ((eqrpamax - eqrpa)/MEC2)
        for i in range(1,7):
            if hfb_qval < 0:
                ps[u'f'+str(i)] = np.zeros(dim)
            else:
                ps_fct = psi_obj.psiFct(i, Zd, A, eqrpamax, eqrpamin)
                psvals = ps_fct(wmax)
                ps[u'f'+str(i)] = psvals

        #--------------------------------------------------------------
        # Sort raw strength by k and add in the prefactors
        #--------------------------------------------------------------
        # Total strength is stored as b={op:{k:cstr_df}} from raw strength objects
        # Initialize all to zero so we can loop over non-existant k values when
        # we compute shape functions C (e.g. k=1,2 for RS0 should be zero)
        al_ops = [u'F', u'GT']
        ff_ops = [u'P', u'R', u'PS0', u'RS0', u'RS1', u'RS2']
        xts    = [u'RS0_PS0', u'R_RS1', u'P_RS1', u'R_P']
        b = {o : {k : np.zeros(dim) for k in range(3)} for o in al_ops+ff_ops+xts}
        ...
        for s in list(self.strengths.values()):
            ...
            if zero_neg_str:
                if self.contour.closed:
                    print(u"WARNING: Do not set negative strength to zero for open contours. Skipping.")
                else:
                    cstr = s.cstr_df_zeroed

            if k_err[k] and genop not in al_ops_raw: continue
        ...
        #--------------------------------------------------------------
        # Calculate the first forbidden shape factors
        #--------------------------------------------------------------
        # Max electron energy and energy related prefactors
        xp   = wmax/3.0 + 0.5*ALPHA*Zd/rad
        xm   = wmax/3.0 - 0.5*ALPHA*Zd/rad

        # Complete shape factor by J. The R index indicates the phase space factor to use
        ...
        C = {jterm : {k : np.zeros(dim) for k in range(3)} for jterm in C_keys}
        ...
        for k in range(3):
            ...
            #--------------------------------------------------------------
            C[u'J0_R1'][k] = \
                    (-2.0/3.0*LAM**2.0*(xp*b[u'RS0'][k] + b[u'RS0_PS0'][k]))*ps[u'f1']
            ...
            #--------------------------------------------------------------
            C[u'J1_R1'][k] = \
                    (-2.0/9.0*(xp*b[u'R'][k] - 2.0*LAM**2.0*xm*b[u'RS1'][k]\
                    + LAM*np.sqrt(2.0)*(xp-xm)*b[u'R_RS1'][k]\
                    - np.sqrt(3.0)*b[u'R_P'][k] - LAM*np.sqrt(6.0)*b[u'P_RS1'][k]))*ps[u'f1']

            C[u'J1_R2'][k] = \
                    (b[u'P'][k] +\
                    1.0/3.0*xp**2.0*b[u'R'][k] +\
                    2.0/3.0*LAM**2.0*xm**2.0*b[u'RS1'][k] +\
                    -8.0/27.0*(LAM**2.0*b[u'RS1'][k] + (LAM/np.sqrt(2.0)*b[u'R_RS1'][k]))*gamma1 +\
                    1.0/27.0*(b[u'R'][k] + 2.0*LAM**2.0*b[u'RS1'][k] +\
                            2.0*np.sqrt(2.0)*LAM*b[u'R_RS1'][k]) +\
                    np.sqrt(2.0/3.0)*(2.0*LAM*xm*b[u'P_RS1'][k] - np.sqrt(2.0)*xp*b[u'R_P'][k] +\
                            (-2.0/np.sqrt(3.0)*LAM*xm*xp*b[u'R_RS1'][k])))*ps[u'f2']

            C[u'J1_R3'][k] = \
                    (4.0/3.0*(np.sqrt(2.0)/3.0*LAM*xp*b[u'R_RS1'][k] +\
                    -2.0/3.0*LAM**2.0*xm*b[u'RS1'][k] +\
            ...
            C[u'J1_R5'][k] = \
                    ...
                    2.0*np.sqrt(2.0)*LAM*b[u'R_RS1'][k]))*ps[u'f5']

            C[u'J1_R6'][k] = \
                    (1.0/27.0*(2.0*b[u'R'][k] + LAM**2.0*b[u'RS1'][k] +\
                    -2.0*np.sqrt(2.0)*LAM*b[u'R_RS1'][k]))*ps[u'f6']
            #--------------------------------------------------------------
            # J = 2 terms
            #--------------------------------------------------------------
        ...
        #--------------------------------------------------------------
        # Group data for various 'total' strengths
        #--------------------------------------------------------------
        # Final list with allowed (multiplied by phase space here) and forbidden terms
        all_sf = {o : {k : b[o][k]*ps[u'f2'] for k in range(3)} for o in al_ops}
        all_sf.update(C)

        # Get quantities which are summed over K and operators
        ...
        for fk in final_keys:
            ...
            for key2sum in final_keys[fk][u'keys']:
                for k2sum in final_keys[fk][u'k']:
                    tmp += all_sf[key2sum][k2sum]
            sf_df[fk] = tmp

        #--------------------------------------------------------------
        # Finalize the calculation by gathering important data and
        # populating instance attributes
        #--------------------------------------------------------------
        # Get location and index of 1st peak. Default is None for closed ctrs
        if not self.contour.closed and not zero_neg_str:
            total_str = np.imag(sf_df[u'Total'].values)
        ...
        if self.e_1stpeak is None:
            ...
            fam_qval = eqrpamax - self.e_1stpeak

        # Dictionary with values sumarizing the calculation
        summary = {u'Z_i'        : Zd-Z_change,
                   u'A'          : A,
                   u'Z_f'        : Zd,
                   u'HFB_Qval'   : hfb_qval,
                   u'E_1stPeak'  : self.e_1stpeak,
                   u'EQRPAmax'   : eqrpamax,
                   u'FAM_Qval'   : fam_qval,
                   u'g_A/g_V'    : LAM,
                   u'g_A'        : GA,
                   ...
                   u'alpha*Z'    : ALPHA*Zd,
                   u'Radius'     : rad,
                   u'Xi'         : (ALPHA*Zd/(2.0*rad)),
                   u'W0_max'     : 1.0 + (hfb_qval/MEC2),
                   u'W0*R'       : ((1.0+hfb_qval)/MEC2*rad),
                   u'half_width' : half_width
                   }
        summary.update(psi_obj._settings)
        summary.update({u'quad':None, u'FAM_ctr':self.contour.name})

        # Store ps, sf and summary
        self.sf_metadict = summary
        self.sf_df = sf_df
        self.ps_df = ps
        ...
        energy_df[u'Re(W0)'] = np.real(wmax)
        energy_df[u'Im(W0)'] = np.imag(wmax)
    ...
    #--------------------------------------------------------------
    def calcShapeFactor(self, psi_obj, hfb_gs):
        """
        Public wrapper for _calcShapeFactor using the raw strength as it
        comes out of pnfam. (zero_neg_str=False).

        Args:
            psi_obj (phaseSpace): The phase space object.
            hfb_gs (hfbthoRun): The ground state hfbthoRun object.
        """
        self._calcShapeFactor(psi_obj, hfb_gs, zero_neg_str=False)

    #--------------------------------------------------------------
    def zeroNegStr(self, psi_obj, hfb_gs):
        """
        Public wrapper for _calcShapeFactor using the adjusted strength which
        has negative compenents set to zero. (zero_neg_str=True). It also uses
        the shape factor calculated from the raw strength to determines the
        location of the first peak (gs-gs transition) and set all strength
        before this point to zero.

        Args:
            psi_obj (phaseSpace): The phase space object.
            hfb_gs (hfbthoRun): The ground state hfbthoRun object.
        """
        # We need the raw strength, so if we don't have it, calculate it
        if not self.is_raw:
            self._calcShapeFactor(psi_obj, hfb_gs, zero_neg_str=False)
        ind_start = self.ind_start

        # Now calculate the zeroed strength (skips looking for 1st peak here)
        self._calcShapeFactor(psi_obj, hfb_gs, zero_neg_str=True)
    ...
    #==============================================================#
    #--------------------------------------------------------------
    def calcBetaRates(self, emin=None, emax=None, quad=None):
        r"""
        ...
        elif quad not in [u'TRAP', u'GAUSS', u'SIMPSON']:
            raise RuntimeError(u"Invalid quadrature requested.")
        ...
        # Get DF in desired range (closed must use whole df)
        if not self.contour.closed:
            eqrpa = self.sf_df_imag[u'Re(EQRPA)'].values
            if emin is None: emin = min(eqrpa)
            if emax is None: emax = max(eqrpa)
            mask = np.r_[eqrpa>=emin] & np.r_[eqrpa<=emax]
            beta_df = beta_df[mask]
            eqrpa = eqrpa[mask]
        elif emin is not None or emax is not None:
            print(u"WARNING: emin/emax provided, but is ignored for closed contours.")
        ...
        if not self.contour.closed:
            ...
            fac  = -0.5
            dzdt = self.contour.ctr_dzdt
            dt   = self.contour.theta
            glwts= self.contour.glwts

        # For open contours, truncate before egs
        for h in beta_df:
            if h in shapeFactor.betaout_keys:
                if quad == u'GAUSS':
                    if not self.contour.use_gauleg:
                        raise RuntimeError(u"Strength points do not lie on a gauss-legendre grid.")
                    ctr_int = fac*sum((beta_df[h]*dzdt)*glwts)
                elif quad == u'TRAP':
                    ctr_int = fac*trapz(beta_df[h]*dzdt, dt)
                elif quad == u'SIMPSON':
                    ctr_int = fac*simps(beta_df[h]*dzdt, dt)

                rate     = (np.log(2)/KAPPA*np.imag(ctr_int))
        ...
        beta_final = pd.DataFrame(betaout)
        beta_final = beta_final[headers]
        beta_final.set_index(u'Contribution', inplace=True)

        self.sf_metadict.update({u'quad':quad, u'FAM_ctr':self.contour.name})
    ...
    def calcCumulativeStr(self, str_df=None, emin=None, emax=None, rate=True):
        ...
        self.sf_metadict.update({u'quad':u'TRAP', u'FAM_ctr':self.contour.name})
        return cumstr_out

    #--------------------------------------------------------------
    def calcTotalGT(self, emin=None, emax=None, zero_neg_str=False):
        """
        Calculate the total GamowTeller strength as GT(K=0) + 2*GT(K=1).
    ...
    def sfSummaryString(self, sd_in):
        ...
        for k in sd_in:
            if sd_in[k] is None:
                sd[k] = (u'{:<10}', u'N/A')
            elif isinstance(sd_in[k], float):
                sd[k] = (u'{:<10.5f}', sd_in[k])
            elif isinstance(sd_in[k], int):
                sd[k] = (u'{:<10d}', sd_in[k])
            else:
                sd[k] = (u'{:<10}', sd_in[k])
        ...
        if sd_in[u'psi_approx'] == u'RATINT':
            ...
            psi_line = [u'polynomial_fit_pts', u'polynomial_order']
        else:
            psi_line = []
        keys_by_line = [[u'FAM_ctr',     u'quad',                  u'half_width'],
                        [u'psi_approx',  u'psi_glpts']+psi_line,
                        [u'Z_i',         u'A',        u'Z_f',      u'HFB_Qval'],
                        [u'FAM_Qval',    u'EQRPAmax', u'E_1stPeak',u'g_A/g_V'],
                        [u'g_A',         u'g_V',      u'M_nucleon',u'alpha*Z'],
                        [u'Radius',      u'Xi',       u'W0_max',   u'W0*R']
                        ]
        ...
        for keys in keys_by_line:
            string = u', '.join([u'{:<10} = '+sd[k][0] for k in keys])
