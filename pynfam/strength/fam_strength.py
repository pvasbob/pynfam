...
class famStrength(object):
    ...
    def getFamList(self, paths, nml_params=None):
        ...
        fam_rep.label = os.path.join(paths.rp(paths.fam_m), self.opname)
    ...
    def checkFamListCtr(self, fam_list):
        ...
        if any([abs(np.real(f.omega-self.contour.ctr_z[i]))>1e-10 or\
            ...
            fam_map    = { np.round(f.omega,10) : f for f in fam_list}
            fam_sorted = [fam_map[np.round(z,10)] for z in self.contour.ctr_z[:self.contour.nr_compute]]
            if any([f.omega != self.contour.ctr_z[i] for i, f in enumerate(fam_list)]):
    ...
    def concatFamTime(self, fam_list):
        ...
        total_time = np.sum(times)/60.0 # minutes
    ...
    def writeStrengthOut(self, dest=u'./', fname=None):
        ...
        if fname is None: fname = self.opname+u'.out'
    ...
    def writeCtrBinary(self, dest=u'./', fname=None):
        ...
        if fname is None: fname = self.opname+u'.out.ctr'
