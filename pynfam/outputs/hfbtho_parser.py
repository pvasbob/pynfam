...
class hfbthoParser(parser):
    ...
    def getSPData(self):
        ...
        times = self.getLineIndices(self.keys['time'])

        try:
            if not (times and pn_data[u'n'][u'start'] and pn_data[u'n'][u'end']\
    ...
    def getQPData(self):
        ...
        try:
            ...
            for t, pn in list(pn_data.items()):
                ...
                if blo_on:
                    b_line_ind = blo_on[np.argmax(np.array(blo_on) > times[self.imin])]
